...
class AcquisitionWorker(QObject):
    ...
    def run(self):
        """
        Main acquisition logic for dusk flats. This method is executed when the thread starts.
        """
        camera = None
        try:
            # --- Initialization ---
            ...
            # --- Phase 1: Wait for sky to darken sufficiently ---
            self.update_status.emit(f"Phase 1: Sky is too bright. Waiting for image mean < {UPPER_BRIGHTNESS_THRESHOLD}...")
            ...
            while self._running:
                ...
                if frame is not None:
                    ...
                    if mean_val < UPPER_BRIGHTNESS_THRESHOLD:
                        self.update_status.emit(f"Phase 1 Complete. Mean is {mean_val:.2f}. Starting acquisition.")
                        break
                time.sleep(0.1)
            ...
            # --- Phase 2: Acquire and average frames until sky is too dark ---
            self.update_status.emit(f"Phase 2: Acquiring frames until mean drops below {LOWER_BRIGHTNESS_THRESHOLD}...")
            bright_frames_count = 0
            sum_image = None
            
            # This loop starts immediately with the frame we just got from Phase 1
            # so we process it first before getting a new one.
            
            # Start sum with the last valid frame from Phase 1
            ...
            while self._running:
                ...
                if latest_frame is not None:
                    ...
                    # Stop condition: if the frame is too dark, we break the loop
                    if latest_mean < LOWER_BRIGHTNESS_THRESHOLD:
                        self.update_status.emit(f"Phase 2 Complete. Final mean ({latest_mean:.2f}) is below threshold.")
                        break
                    
                    # Add the new valid frame to the sum
            ...
            if bright_frames_count == 0:
                self.finished.emit("Warning: No frames were collected. Cannot save.")
                return
...
class StatsWindow(QMainWindow):
    ...
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Automated Flat Field Acquisition (Dusk Mode)")
    ...
    def update_image_view(self, frame):
        # Transpose the image for correct orientation in pyqtgraph
