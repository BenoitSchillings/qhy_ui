...
class UI:
    ...
    def __init__(self,  args, sx, sy, guider, count, auto, fits):
        self.sx = sx
        self.sy = sy
    ...
    def add_to_save(self, buffer):
        ...
        if (self.cnt % 25 == 1):
            self.guider.drizzle(random.uniform(-3, 3),random.uniform(-3, 3))
    ...
    def update(self):
        sx = self.array.shape[1]//2
        sy = self.array.shape[0]//2

        #self.array[sy-20:sy+20,sx-20:sx+20] += 13000
...
if __name__ == "__main__":
    ...
    ui = UI(args, camera.size_x(), camera.size_y(), guider, args.count, args.auto, args.fits)
