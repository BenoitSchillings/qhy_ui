...
#x0,y0 to x1,y1 is the motion of N in the X AXIS
#x2,y2 is after the motion of N in the Y axis. ref position is based on x1, y1
...
#x2,y2 is after the motion of N in the Y axis. ref position is based on x1, y1
    ...
    def handle_guide_ao(self, x, y):
        if self.guide_inited_ao <= 0:
            ...
            self.fmove_ao(-1.0*ao_dx, 1.0*ao_dy)  # Negative because we want to move opposite to the error
    ...
    def calculate_actuator_movements(self, dx, dy):
        """
        Calculate the required X and Y actuator movements for a given desired motion.
        
        Parameters:
        dx, dy: Desired motion of the center
        dx1, dy1: Motion caused by a unit movement of the X actuator
        dx2, dy2: Motion caused by a unit movement of the Y actuator
        
        Returns:
        x_movement, y_movement: Required movements of the X and Y actuators
        """
        # Create the calibration matrix
        calibration_matrix = np.array([[self.ao_dx1, self.ao_dx2],
                                       [self.ao_dy1, self.ao_dy2]])
        
        # Create the desired motion vector
        desired_motion = np.array([dx, dy])
        
        # Solve the linear system
        actuator_movements = np.linalg.solve(calibration_matrix, desired_motion)
        
        return actuator_movements[0], actuator_movements[1]
